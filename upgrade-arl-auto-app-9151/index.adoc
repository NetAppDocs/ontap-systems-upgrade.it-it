...
[role="lead"]
Esistono diversi metodi di rilocazione aggregata (ARL) per l'aggiornamento dell'hardware del controller. Questa procedura ARL descrive come aggiornare una coppia HA di controller nei sistemi di storage AFF e FAS introdotti in ONTAP 9.15.1 o versioni successive con nuovi controller, mantenendo i dischi esistenti e i dati memorizzati su di essi.
...
Durante un aggiornamento del controller, il controller esistente viene sostituito con un altro controller che utilizza una piattaforma più recente o più potente. Questo contenuto fornisce i passaggi per la procedura parzialmente automatizzata, che utilizza controlli automatici di raggiungibilità delle porte di rete per semplificare ulteriormente l'esperienza di aggiornamento del controller.

.Quali sono le prossime novità?
Dopo aver appreso di questa procedura di aggiornamento ARL, link:decide_to_use_the_aggregate_relocation_guide.html["decidere se utilizzare questa procedura"].
