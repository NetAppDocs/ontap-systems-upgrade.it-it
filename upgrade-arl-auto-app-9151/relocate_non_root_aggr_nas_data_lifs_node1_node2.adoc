...
summary: 'Sposta gli aggregati non root e i LIF dati NAS dal nodo1 al nodo2 e sposta le risorse del nodo1 sul nodo3 quando esegui l"aggiornamento ai controller introdotti in ONTAP 9.15.1 o versioni successive usando i comandi  `system controller replace`.' 
---
...
Dopo aver spostato gli aggregati non root e i LIF dei dati NAS di proprietà del node1 al node2, link:relocate-failed-or-vetoed-aggr-node2.html["spostare gli aggregati non riusciti o rifiutati al node2"].
