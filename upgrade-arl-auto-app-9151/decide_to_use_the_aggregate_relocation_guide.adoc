...
[role="lead"]
Esistono diversi metodi di rilocazione aggregata (ARL) per l'aggiornamento dell'hardware del controller. Questa procedura ARL descrive come aggiornare una coppia HA di controller nei sistemi di storage AFF e FAS introdotti in ONTAP 9.15.1 o versioni successive con nuovi controller, mantenendo i dischi e i dati archiviati esistenti. Si tratta di una procedura complessa che dovrebbe essere utilizzata solo da amministratori ONTAP esperti.
