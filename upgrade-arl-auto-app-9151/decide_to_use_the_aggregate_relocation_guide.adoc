...
summary: 'Decidi se usare ARL per aggiornare ai controller introdotti in ONTAP 9.15.1 o versioni successive usando i comandi  `system controller replace`.' 
---
...
[role="lead"]
Esistono diversi metodi di riallocazione aggregata (ARL) per l'aggiornamento dell'hardware del controller. Questa procedura ARL descrive come aggiornare una coppia HA di controller nei sistemi di storage AFF e FAS ai controller introdotti in ONTAP 9.15.1 o versioni successive, mantenendo i dischi esistenti e i dati memorizzati sui dischi. Questa è una procedura complessa che dovresti usare solo se sei un amministratore ONTAP esperto.
...
|===
+
Per eseguire un aggiornamento elencato nella tabella precedente, vedi link:other_references.html["Riferimenti"] per il collegamento a _Usa i comandi "system controller replace" per aggiornare i modelli di controller nello stesso chassis_.
...
Dopo aver deciso di utilizzare questa procedura ARL, link:required_tools_and_documentation.html["Scopri gli strumenti e la documentazione necessari"].
