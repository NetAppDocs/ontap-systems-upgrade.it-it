...
summary: 'Installa e avvia node3 e installa il software ONTAP quando esegui l"upgrade ai controller introdotti in ONTAP 9.15.1 o versioni successive usando i comandi  `system controller replace`.' 
---
...
. Quando node3 si ferma al menu di avvio, riassegnare i dischi di node1 a node3 eseguendo il seguente comando su node3:
+
...
. Premere  `Ctrl-C` durante autoboot per arrestare il nodo al  `LOADER>` prompt.
...
Dopo aver installato e avviato node3, link:verify_node3_installation.html["verificare l'installazione di node3"].
