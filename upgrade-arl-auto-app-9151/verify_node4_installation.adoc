...
summary: 'Verifica che le porte fisiche del nodo2 siano mappate correttamente sul nodo4 e ripristina la configurazione di rete sul nodo4 quando esegui l"aggiornamento ai controller introdotti in ONTAP 9.15.1 o versioni successive utilizzando i comandi  `system controller replace`.' 
---
...
[role="lead"]
È necessario verificare che le porte fisiche del nodo2 siano mappate correttamente sulle porte fisiche del nodo4. Ciò consente al nodo4 di comunicare con gli altri nodi del cluster e con la rete dopo l'aggiornamento.
...
La configurazione fisica delle porte può variare a seconda del modello dei nodi. All'avvio del nuovo nodo, ONTAP tenta di determinare quali porte devono ospitare le LIF del cluster per entrare automaticamente in quorum.

Se le porte fisiche sul nodo2 non corrispondono direttamente alle porte fisiche sul nodo4, la sezione successiva <<restore_network_configuration_on_node4_9151,Ripristinare la configurazione di rete sul nodo 4>> deve essere utilizzata per ripristinare la connettività di rete.
...
A questo punto della procedura, l'operazione si interrompe poiché node4 si unisce al quorum.



[[step-1-verify-that-node4-is-in-quorum]]
== Passaggio 1: Verificare che node4 sia in quorum

Verificare che il nodo4 sia in quorum. Quindi, riprendere l'operazione di sostituzione del controller.
...
. La sostituzione del controller si interrompe per consentire l'intervento e visualizza il seguente messaggio:
+
....
...
== Passaggio 2: Ripristinare la configurazione di rete sul nodo4
...
+
Durante la procedura di ripristino della raggiungibilità, ONTAP tenta di assegnare le porte ai domini di broadcast corretti. Tuttavia, se ONTAP non riesce a determinare la raggiungibilità di una porta e la porta non appartiene a nessuno dei domini di broadcast esistenti, ONTAP crea nuovi domini di broadcast per queste porte.
...
Dopo aver verificato l'installazione di node4, link:restore_key-manager_config_node4.html["ripristinare la configurazione del key-manager sul node4"].
