...
summary: 'Scopri se puoi eseguire l"aggiornamento ai controller introdotti in ONTAP 9.15.1 o versioni successive utilizzando i comandi  `system controller replace`.' 
---
...
[role="lead"]
Per capire se puoi usare ARL per aggiornare a una coppia di controller introdotti in ONTAP 9.15.1 o versioni successive, dipende dalla piattaforma e dalla configurazione sia dei controller originali che di quelli sostitutivi.
...
Se si verificano problemi durante l'aggiornamento dei controller, vederelink:aggregate_relocation_failures.html["Risolvere i problemi"] per maggiori informazioni e possibili soluzioni.

Se non si riesce a trovare una soluzione al problema riscontrato, contattare il supporto tecnico.

.Quali sono le prossime novità?
Dopo aver esaminato le linee guida per l'aggiornamento dei controller con ARL, link:overview_of_the_arl_upgrade.html["Scopri la sequenza di aggiornamento ARL"].
