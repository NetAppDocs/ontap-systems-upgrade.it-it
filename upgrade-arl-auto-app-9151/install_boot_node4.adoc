...
summary: 'Installa e avvia node4 e installa il software ONTAP quando esegui l"upgrade ai controller introdotti in ONTAP 9.15.1 o versioni successive usando i comandi  `system controller replace`.' 
---
...
. [[auto_check_4_step27]]Al prompt LOADER, avviare il nodo:
+
`boot_ontap menu`
...
Dopo aver installato e avviato node4, link:verify_node4_installation.html["verificare l'installazione di node4"].
