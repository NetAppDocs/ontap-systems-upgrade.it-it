...
summary: 'Verifica che le porte fisiche di node1 siano mappate correttamente su node3 e ripristina la configurazione di rete su node3 quando esegui l"aggiornamento ai controller introdotti in ONTAP 9.15.1 o versioni successive usando i comandi  `system controller replace`.' 
---
...
[role="lead"]
È necessario verificare che le porte fisiche di node1 siano mappate correttamente sulle porte fisiche di node3. Ciò consente a node3 di comunicare con gli altri nodi del cluster e con la rete dopo l'aggiornamento.
...
Se le porte fisiche sul nodo1 non corrispondono direttamente alle porte fisiche sul nodo3, la sezione successiva <<restore_network_configuration_on_node3_9151,Ripristinare la configurazione di rete sul nodo 3>> deve essere utilizzata per ripristinare la connettività di rete.
...
A questo punto della procedura, l'operazione si interrompe poiché node3 si unisce al quorum.



[[step-1-verify-that-node3-is-in-quorum]]
== Passaggio 1: Verificare che node3 sia in quorum

Verificare che il nodo3 sia in quorum. Quindi, riprendere l'operazione di sostituzione del controller.
...
== Passaggio 2: Ripristinare la configurazione di rete sul nodo3
...
Dopo aver verificato l'installazione di node3, link:restore_key-manager_configuration_node3.html["ripristinare la configurazione del key-manager sul nodo3"].
