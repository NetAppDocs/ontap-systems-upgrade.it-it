...
summary: 'Se necessario, ripristina la configurazione del key-manager su node3 quando esegui l"aggiornamento ai controller introdotti in ONTAP 9.15.1 o versioni successive utilizzando i comandi  `system controller replace`.' 
---
...
Dopo aver ripristinato la configurazione del key-manager sul nodo3, link:move_non-root_aggr_and_nas_data_lifs_node1_from_node2_to_node3.html["Spostare gli aggregati non root e i LIF di dati NAS di proprietà del node1 dal node2 al node3"].
