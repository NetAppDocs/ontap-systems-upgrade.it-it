...
summary: 'Verifica che i LIF dati NAS di node1 su node2 vengano spostati da node2 a node3 quando esegui l"aggiornamento ai controller introdotti in ONTAP 9.15.1 o versioni successive usando i comandi  `system controller replace`.' 
---
...
Durante la procedura di aggiornamento, i LIF remoti gestiscono il traffico verso le LUN SAN. Lo spostamento dei LIF SAN non è necessario per l'integrità del cluster o dei servizi durante l'aggiornamento. I LIF SAN non vengono spostati a meno che non sia necessario mapparli su nuove porte. Verificare che i LIF siano integri e posizionati sulle porte appropriate dopo aver portato online node3.
...
Dopo aver spostato gli aggregati non root e i LIF di dati NAS di proprietà del nodo1 dal nodo2 al nodo3, link:relocate_non_root_aggr_nas_lifs_from_node2_to_node3.html["spostare gli aggregati non root e le LIF dei dati NAS dal nodo2 al nodo3"].
