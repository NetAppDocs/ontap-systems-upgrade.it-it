...
[role="lead"]
Le porte fisiche sul nodo2 devono essere mappate correttamente sulle porte fisiche del nodo4. Ciò consente al nodo4 di comunicare con gli altri nodi del cluster e con la rete dopo l'upgrade.
...
Le impostazioni delle porte possono variare a seconda del modello dei nodi. È necessario rendere compatibili la configurazione delle porte e dei LIF del nodo originale con quella che si intende utilizzare per il nuovo nodo. Questo perché il nuovo nodo riproduce la stessa configurazione all'avvio, ovvero, all'avvio del nodo4, Data ONTAP tenta di ospitare i LIF sulle stesse porte utilizzate sul nodo2.

Pertanto, se le porte fisiche sul nodo2 non corrispondono direttamente alle porte fisiche sul nodo4, sono necessarie modifiche alla configurazione software per ripristinare la connettività del cluster, della gestione e della rete dopo il boot. Inoltre, se le porte del cluster sul nodo2 non corrispondono direttamente alle porte del cluster sul nodo4, il nodo4 potrebbe non rientrare automaticamente nel quorum al riavvio finché non viene apportata una modifica alla configurazione software per ospitare i LIF del cluster sulle porte fisiche corrette.
...
+
Per ulteriori informazioni, fare riferimento a link:other_references.html["Riferimenti"] per collegarsi a _Network Management_ o al _ONTAP 9 Command reference_, e andare a _Configuring failover settings on a LIF_.
