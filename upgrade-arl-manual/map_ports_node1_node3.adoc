...
[role="lead"]
È necessario assicurarsi che le porte fisiche su node1 corrispondano correttamente alle porte fisiche su node3. Ciò consente a node3 di comunicare con gli altri nodi del cluster e con la rete dopo l'aggiornamento.
...
È necessario rendere compatibili la porta e la configurazione LIF del nodo originale con la configurazione che si intende utilizzare per il nuovo nodo. Questo perché il nuovo nodo riproduce la stessa configurazione all'avvio, il che significa che quando si avvia node3, ONTAP tenta di ospitare le LIF sulle stesse porte utilizzate su node1.

Pertanto, se le porte fisiche su node1 non corrispondono direttamente alle porte fisiche su node3, sono necessarie modifiche alla configurazione software per ripristinare la connettività del cluster, della gestione e della rete dopo il boot. Inoltre, se le porte del cluster su node1 non corrispondono direttamente alle porte del cluster su node3, node3 potrebbe non rientrare automaticamente nel quorum al riavvio finché non viene apportata una modifica alla configurazione software per ospitare le LIF del cluster sulle porte fisiche corrette.
...
|===
+
Consultare link:record_node1_information.html["Registrare le informazioni del nodo 1"] per i passaggi per ottenere queste informazioni.
...
----
+
Vai a link:other_references.html["Riferimenti"] per collegarti a _Network Management_ o al _ONTAP 9 Command reference_ per ulteriori informazioni.
