...
* Questa procedura si applica alle configurazioni MetroCluster a quattro e otto nodi che eseguono ONTAP 9.5 e versioni precedenti. Per le configurazioni MetroCluster con ONTAP 9.6 e versioni successive, visitare il sito Web all'indirizzo link:other_references.html["Riferimenti"] Collegamento a _utilizzo dei comandi di sostituzione del controller di sistema" per aggiornare l'hardware del controller che esegue ONTAP 9.5 a ONTAP 9.7_.
