...
[role="lead"]
È necessario verificare che le porte fisiche del nodo2 corrispondano correttamente alle porte fisiche del nodo4. Ciò consente al nodo4 di comunicare con gli altri nodi del cluster e con la rete dopo l'aggiornamento.
...
Pertanto, se le porte fisiche su node2 non corrispondono direttamente alle porte fisiche su node4, sono necessarie modifiche alla configurazione software per ripristinare la connettività del cluster, della gestione e della rete dopo il boot. Inoltre, se le porte del cluster su node2 non corrispondono direttamente alle porte del cluster su node4, node4 potrebbe non rientrare automaticamente nel quorum al riavvio finché non viene apportata una modifica alla configurazione software per ospitare le LIF del cluster sulle porte fisiche corrette.
...
+
Fare riferimento a link:other_references.html["Riferimenti"] per collegarsi a _Network Management_ o al _ONTAP 9 Command reference_ e vedere _Configuring failover settings on a LIF_ per ulteriori informazioni.
