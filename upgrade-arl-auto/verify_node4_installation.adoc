...
CAUTION: In questa fase, la configurazione MetroCluster non è in uno stato normale e potrebbero esserci errori da risolvere. Vedi link:verify_health_of_metrocluster_config.html["Verificare lo stato della configurazione MetroCluster"].
...
Se si utilizza NetApp Aggregate Encryption (NAE) o NetApp Volume Encryption (NVE) per crittografare i volumi sul sistema che si sta aggiornando, la configurazione di crittografia deve essere sincronizzata con i nuovi nodi. Se non si ripristina key-manager, quando si spostano gli aggregati di node2 da node3 a node4 tramite ARL, i volumi crittografati vengono messi offline.
