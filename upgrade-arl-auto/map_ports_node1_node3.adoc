...
[role="lead"]
È necessario verificare che le porte fisiche su node1 corrispondano correttamente alle porte fisiche su node3. Ciò consente a node3 di comunicare con gli altri nodi del cluster e con la rete dopo l'aggiornamento.
...
Le impostazioni delle porte possono variare a seconda del modello dei nodi. È necessario rendere la configurazione delle porte e dei LIF sul nodo originale compatibile con l'utilizzo e la configurazione previsti per il nuovo nodo. Questo perché il nuovo nodo riproduce la stessa configurazione all'avvio, il che significa che quando si avvia node3, ONTAP tenta di ospitare i LIF sulle stesse porte utilizzate su node1.
...
+
Fare riferimento a link:other_references.html["Riferimenti"] per collegarsi a _Network Management_ o al _ONTAP 9 Command reference_ per ulteriori informazioni.
