...
.Fasi
. [[reassign-node2-node4-step1]]Verificare che node2 si sia fermato al menu di avvio e riassegnare i dischi di node2 a node4:
+
...
----
====
. Se il sistema entra in un ciclo di riavvio con il messaggio `no disks found` , questo perché ha reimpostato le porte sulla modalità di destinazione e quindi non è in grado di vedere alcun disco.  Eseguire<<auto_check_4_step3,Fase 3>> Attraverso<<auto_check_4_step8,Fase 8>> su node4 per risolvere questo problema.
