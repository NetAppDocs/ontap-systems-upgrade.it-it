...
.. Durante la procedura di ripristino della raggiungibilità, ONTAP tenta di inserire le porte nei domini di broadcast corretti. Tuttavia, se ONTAP non riesce a determinare la raggiungibilità di una porta e la porta non corrisponde a nessuno dei domini di broadcast esistenti, ONTAP crea nuovi domini di broadcast per queste porte. Se necessario, è possibile eliminare i domini di broadcast appena creati se tutte le porte che li compongono diventeranno porte membri dei gruppi di interfaccia. Eliminare i domini di broadcast:
+
