...
Le LUN e i volumi sono ancora fisicamente ospitati sui vecchi nodi. Poiché le LUN vengono segnalate solo sulle interfacce dei vecchi nodi, l'host mostra solo i percorsi sui vecchi nodi. Per verificarlo, eseguire i comandi  `sanlun lun show -p` e  `multipath -ll -d` sull'host ed esaminare gli output dei comandi.
...
link:upgrade-create-aggregate-move-volumes.html["Creare un aggregato e spostare i volumi nei nuovi nodi"]
