...
Se il cluster si trova in un ambiente SAN, è necessario eliminare dai nodi originali tutti i LIF SAN non più necessari prima di poter rimuovere i nodi originali dal cluster.
...
link:upgrade-remove-original-nodes.html["Rimuovere i nodi originali dal cluster"]
