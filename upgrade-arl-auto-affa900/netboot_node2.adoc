...
summary: Dopo aver scambiato il modulo controller AFF A900 node2 e il modulo NVRAM corrispondenti, potrebbe essere necessario avviarli in rete. 
---
= Node2 NetBoot
:hardbreaks:
:allow-uri-read: 
...
. Attendere che il nodo 2 sia ora in esecuzione sul modulo controller AFF A900 o FAS9500 avvii e visualizzare le opzioni del menu di avvio come mostrato nell'output seguente:
+
[listing]
----
...
+
Per il node2, il `partner-sysid` Deve essere quello del node1 che si sta eseguendo l'aggiornamento a un sistema AFF A900 o FAS9500.
