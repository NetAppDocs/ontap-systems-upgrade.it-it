...
Se il sistema esistente dispone di una porta di gestione (e0M) e di una porta BMC. Sul sistema di sostituzione, le porte e0M e BMC sono combinate e accessibili attraverso la porta "chiave". Prima di eseguire la connessione al sistema sostitutivo, è necessario assicurarsi che le porte e0M e BMC siano connesse allo stesso switch e alla stessa subnet del sistema esistente.
