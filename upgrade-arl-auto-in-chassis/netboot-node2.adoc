...
summary: Quando usi ARL per aggiornare i modelli di controller nello stesso chassis, devi avviare in rete node2 dopo aver sostituito il modulo di sistema corrispondente. 
---
= Node2 NetBoot
:hardbreaks:
:allow-uri-read: 
...
Dopo aver avviato il nodo2 tramite rete, link:cable-node2-for-shared-cluster-HA-storage.html["cavo node2 per cluster condiviso-HA e storage"].
