...
[role="lead"]
Node2 con i moduli sostitutivi è ora pronto per l'avvio. I moduli sostitutivi supportati sono elencati nella link:decide-to-use-the-aggregate-relocation-guide.html#supported-systems-in-chassis["matrice dei sistemi supportati"].
...
+
Inserisci la passphrase e le informazioni di backup che hai annotato in precedenza nella procedura. Vedi link:complete-preparation-for-upgrade.html#manage-storage-encryption-using-okm["Gestire la crittografia dello storage utilizzando Onboard Key Manager"].
...
. Quando il nodo si ferma al menu di avvio, riassegnare i vecchi dischi node2 al node2 sostitutivo eseguendo il seguente comando su node2:
+
...
Dopo aver avviato node2 con i moduli di sistema sostitutivi, link:verify-node2-installation.html["verificare l'installazione di node2"].
