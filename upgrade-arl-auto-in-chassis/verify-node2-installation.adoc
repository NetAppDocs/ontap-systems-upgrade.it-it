...
== Passaggio 1: Verificare che il nodo2 sia in quorum

Verificare che il nodo2 sia in quorum. Quindi, riprendere l'operazione di sostituzione del controller.
...
. Con la sostituzione del controller in pausa, procedere a <<restore_network_configuration_on_node2,Ripristinare la configurazione di rete sul nodo 2>>.




== Passaggio 2: Ripristinare la configurazione di rete sul nodo2
...
Dopo aver verificato l'installazione di node2, link:restore-key-manager-config-node2.html["ripristinare la configurazione del key-manager sul nodo2"].
