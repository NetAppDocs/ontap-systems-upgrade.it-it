...
Se si sta eseguendo l'aggiornamento a una configurazione di cluster senza switch a due nodi, è possibile saltare questa sezione e passare a link:move-non-root-aggr-and-nas-data-lifs-back-to-node2.html["Riportare al nodo gli aggregati non root e le LIF dei dati NAS 2"].
...
| `Up` | Vai a link:move-non-root-aggr-and-nas-data-lifs-back-to-node2.html["Riportare al nodo gli aggregati non root e le LIF dei dati NAS 2"]. 


| `Down` | Vai a<<verify-rcf-step3,Fase 3>> . 
|===
. [[verify-rcf-step3]]Controllare le porte del cluster e commutare RCF.
+
...
Dopo aver verificato la configurazione RCF link:move-non-root-aggr-and-nas-data-lifs-back-to-node2.html["spostare gli aggregati non root e i LIF dei dati NAS di nuovo nel nodo2"].
