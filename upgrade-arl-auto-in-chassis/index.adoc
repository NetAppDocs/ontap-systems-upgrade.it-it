...
* Se si esegue l'aggiornamento da un sistema esistente con tempi di inattività inclusi nellink:decide_to_use_the_aggregate_relocation_guide.html#supported-systems-in-chassis["matrice dei sistemi supportati"] , è possibile aggiornare l'hardware del controller spostando lo storage o contattando l'assistenza tecnica.  Fare riferimento alink:other_references.html["Riferimenti"] per collegarsi a _Aggiorna spostando volumi o storage_.
