...
[role="lead"]
Esistono diversi metodi di rilocazione aggregata (ARL) per aggiornare l'hardware del controller. Questa procedura descrive come aggiornare i controller di storage in una coppia HA convertendo il sistema esistente nel sistema sostitutivo, mantenendo lo chassis, i dischi esistenti e i dati memorizzati sui dischi.
...
NOTE: I termini *node1* e *node2* sono utilizzati solo come riferimento ai nomi dei nodi in questo documento. Quando si segue la procedura, è necessario utilizzare i nomi effettivi dei propri nodi.
...
È necessario considerare fattori importanti prima di iniziare l'aggiornamento.


NOTE: È necessario esaminare tutte le seguenti informazioni importanti.

* Questa procedura è complessa e presuppone che tu abbia competenze avanzate di amministrazione ONTAP. Dovresti leggere e comprendere le link:guidelines-for-upgrading-controllers-with-arl.html["linee guida per l'aggiornamento dei controller"] e link:overview-of-the-arl-upgrade.html["Sequenza di aggiornamento ARL"] prima di iniziare l'aggiornamento.
...
* Se si dispone di uno switch non supportato dalla versione di ONTAP e dal sistema sostitutivo a cui si sta effettuando l'aggiornamento, fare riferimento a link:other-references.html["Riferimenti"] per il collegamento a _Hardware Universe_.
...
* Se si esegue l'aggiornamento da un sistema esistente con tempi di inattività inclusi nel link:decide-to-use-the-aggregate-relocation-guide.html#supported-systems-in-chassis["matrice dei sistemi supportati"], è possibile aggiornare l'hardware del controller spostando lo storage o contattando il supporto tecnico. Fare riferimento a link:other-references.html["Riferimenti"] per il collegamento a _Aggiornamento tramite spostamento di volumi o storage_.




[[automated-controller-upgrade-process]]
== Processo automatizzato di aggiornamento del controller
...
Dopo aver appreso di questa procedura di aggiornamento ARL, link:decide-to-use-the-aggregate-relocation-guide.html["decidere se utilizzare questa procedura"].
