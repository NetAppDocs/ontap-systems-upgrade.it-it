...
summary: Collega le connessioni del cluster, della coppia HA, dello storage e dei dati che erano precedentemente collegate al nodo AFF A800 node1 al nuovo nodo AFF A90 o AFF A70 appena installato. 
---
...
| HA | Non richiesto | Le porte HA non sono necessarie per l'aggiornamento del nodo 1 | Le porte HA non sono necessarie per l'aggiornamento del nodo 1 
...
Dopo aver cablato il nodo1 per il cluster-HA condiviso e lo storage, link:boot-node1-with-a900-controller-and-nvs.html["Avviare il nodo1 con i moduli di sistema sostitutivi"].
