...
== Passaggio 1: Preparare i nodi

Avviare la procedura di sostituzione del controller ed eseguire i controlli preliminari. Se necessario, determinare il tipo di unità a crittografia automatica in uso.
...
....
. Seleziona `y`. Vedrai il seguente output:
+
....
...
| Verifica della versione dell'immagine | Verifica che i nodi sostituiti eseguano la stessa versione di ONTAP. Se le versioni dell'immagine ONTAP sono diverse, l'attività fallisce. I nuovi nodi devono avere la stessa versione di ONTAP 9.x installata che è installata sui nodi originali. Se i nuovi nodi hanno una versione diversa di ONTAP installata, è necessario eseguire il netboot dei nuovi controller dopo averli installati. Per istruzioni su come aggiornare ONTAP, fare riferimento a link:other-references.html["Riferimenti"] per collegarsi a _Upgrade ONTAP_. 
...
| Verifica dello stato del disco | Se uno dei nodi da sostituire presenta dischi mancanti o guasti, l'attività fallisce. Se mancano dei dischi, fare riferimento a link:other-references.html["Riferimenti"] _Gestione di dischi e aggregati con la CLI_, _Gestione dello storage logico con la CLI_ e _Gestione delle coppie HA_ per configurare lo storage per la coppia HA. 
