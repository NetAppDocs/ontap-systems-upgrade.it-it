...
---
= Sostituire il modulo controller AFF A800 o AFF C800 del nodo1
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
+
Se non interrompi AUTOBOOT in tempo e il nodo1 inizia l'avvio, attendi il prompt e premi Ctrl-C per accedere al menu di boot. Dopo che il nodo si è fermato nel menu di boot, utilizza l'opzione  `8` per riavviare il nodo e interrompere AUTOBOOT durante il riavvio.
...
Dopo aver installato il modulo controller AFF A90, AFF A70 o AFF C80, link:netboot-node1.html["netboot node1"].
