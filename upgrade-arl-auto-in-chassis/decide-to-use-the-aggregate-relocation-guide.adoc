...
summary: Decidere se utilizzare la procedura ARL che aggiorna i modelli di controller nello stesso chassis, i dischi e i dati memorizzati sui dischi. 
---
...
[role="lead"]
Esistono diversi metodi di rilocazione aggregata (ARL) per l'aggiornamento dell'hardware del controller. Questo contenuto descrive come aggiornare i controller di storage in una coppia HA convertendo il sistema esistente nel sistema sostitutivo, mantenendo lo chassis esistente, i dischi e i dati memorizzati sui dischi. Dovresti utilizzare questa complessa procedura solo se sei un amministratore ONTAP esperto.
...
+
Per aggiornare una configurazione IP MetroCluster, vedere link:other-references.html["Riferimenti"] per il collegamento al contenuto _MetroCluster Upgrade and Expansion_.
...
La tabella seguente mostra la matrice dei sistemi supportati per eseguire un aggiornamento del controller convertendo il sistema esistente nel sistema sostitutivo, mantenendo lo chassis, i dischi esistenti e i dati in essi memorizzati.


CAUTION: Questa procedura si applica esclusivamente alle seguenti configurazioni di aggiornamento. Non utilizzare questa procedura per eseguire un aggiornamento tra altre combinazioni di sistema. Per tutti gli altri modelli di controller, fare riferimento a link:other-references.html["Riferimenti"] _Usa i comandi "system controller replace" per aggiornare l'hardware del controller che esegue ONTAP 9.8 o versioni successive_ e _Utilizzo della rilocazione aggregata per aggiornare manualmente l'hardware del controller che esegue ONTAP 9.8 o versioni successive_.
...
| AFF A70 ^1^ | AFF A90 | 9.18.1 e versioni successive 


| AFF A30 ^1^ | AFF A50 | 9.18.1 e versioni successive 


| AFF A20 ^1^ | AFF A30 ^3^, AFF A50 | 9.18.1 e versioni successive 


| AFF C30 ^1^ | AFF C60 | 9.18.1 e versioni successive 


| FAS70 ^1^ | FAS90 | 9.18.1 e versioni successive 
...
====
^3^ Ci sono determinati requisiti di connessione delle porte per un aggiornamento da AFF A20 ad AFF A30 in una configurazione di cluster senza switch a due nodi:

* AFF A20 supporta X60132A (4-port 10/25GbE CX7) e X60131A (2-port 10/25GbE CX6-LX) per l'interconnessione del cluster.
* AFF A30 supporta solo X60130A (2-port 100GbE CX6-DX) per l'interconnessione del cluster.
* Usa una scheda 10/25GbE temporanea per collegare temporaneamente l'AFF A20 all'AFF A30. Una scheda 10/25GbE a 4 o 2 porte supporta il clustering sull'AFF A20. Si consiglia di usare la stessa scheda sia nell'AFF A20 che nell'AFF A30. Se non hai una nuova scheda, puoi usare la scheda dell'AFF A20 come interconnessione cluster temporanea.
* Quando AFF A30 node3 entra in funzione, imposta le porte HA IC su 100GbE e tenta di posizionare le LIF del cluster sulle porte 10/25GbE. ONTAP rileva queste porte come connesse ad AFF A20. Se ONTAP non rileva automaticamente queste porte, puoi modificare manualmente le porte temporanee del cluster. Poi, se necessario, puoi spostare le LIF del cluster sulle porte temporanee del cluster.
* Dopo aver aggiornato AFF A20 node1 e AFF A20 node2 va giù in preparazione per l'upgrade, devi rimuovere le connessioni dalle porte cluster temporanee 10/25GbE e usare solo le porte cluster condivise 100GbE. Quando AFF A30 node3 e node4 sono collegati tramite porte 100GbE, le connessioni si attivano e puoi completare l'upgrade.
