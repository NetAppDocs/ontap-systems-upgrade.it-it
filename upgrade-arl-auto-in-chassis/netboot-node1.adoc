...
Non è possibile verificare la versione di ONTAP installata sul supporto di avvio del modulo controller sostitutivo, a meno che non sia installato in uno chassis e acceso. La versione di ONTAP sul supporto di avvio del sistema sostitutivo deve essere la stessa della versione di ONTAP in esecuzione sul vecchio sistema che si sta aggiornando e sia l'immagine di avvio primaria che quella di backup sul supporto di avvio devono corrispondere. Per verificare la versione minima di ONTAP supportata per l'aggiornamento, consultare la link:decide-to-use-the-aggregate-relocation-guide.html#supported-systems-in-chassis["matrice dei sistemi supportati"].
...
* Scarica i file di sistema necessari per il tuo sistema e la versione corretta di ONTAP dal _NetApp Support Site_. Fai riferimento a link:other-references.html["Riferimenti"] per collegarti al _NetApp Support Site_.
...
.Fasi
. Fare riferimento a link:other-references.html["Riferimenti"] per collegarsi al _NetApp Support Site_ e scaricare i file utilizzati per eseguire il netboot del sistema.
...
+
.. Al prompt successivo, eseguire il  `wipeconfig` comando. Quindi selezionare Invio:
+
[listing]
----
Please choose one of the following:
...
Dopo l'avvio di rete del nodo1, link:cable-node1-for-shared-cluster-HA-storage.html["cavo nodo1 per cluster-HA condiviso e storage"].
