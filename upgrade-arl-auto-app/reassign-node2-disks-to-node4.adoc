...
.Fasi
. [[reassign-node2-node4-app-step1]]Verificare che node2 si sia fermato al menu di avvio e riassegnare i dischi di node2 a node4:
+
...
NOTE: Nell'esempio di output della console riportato sopra, ONTAP richiede il nome del nodo partner se il sistema utilizza dischi con Advanced Disk Partitioning (ADP).
...
+
** Eseguire<<reassign-node2-node4-app-step3,Fase 3>> A<<reassign-node2-node4-app-step8,Fase 8>> sul nodo 4
** Vai alla sezionelink:verify_node4_installation.html["Verificare l'installazione di node4"]


. [[reasign-node2-node4-app-step3]]premere Ctrl-C durante l'OPERAZIONE per arrestare il nodo al prompt Loader>.
...
[NOTE]
====
Se si esegue l'aggiornamento da un sistema che supporta dischi esterni a un sistema che supporta anche dischi esterni, visitare il sito <<reassign-node2-node4-app-step8,Fase 8>>.

Se si esegue l'aggiornamento da un sistema che utilizza dischi esterni a un sistema che supporta dischi interni ed esterni, ad esempio un sistema AFF A800, visitare il sito <<reassign-node2-node4-app-step9,Fase 9>>.

====
. [[reasign-node2-node4-app-step8]]]al prompt di Loader, avviare:
+
`boot_ontap menu`
...
NOTE: In questa fase potrebbe verificarsi un errore. Se si verifica un errore, ripetere i passaggi secondari in <<reassign-node2-node4-app-step8,Fase 8>> fino a quando il sistema non si avvia normalmente.



. [[reassign-node2-node4-app-step9]] Se si esegue l'aggiornamento da un sistema con dischi esterni a un sistema che supporta dischi interni ed esterni (ad esempio, sistemi AFF A800 ), impostare l'aggregato node2 come aggregato radice per garantire che node4 si avvii dall'aggregato radice di node2.  Per impostare l'aggregato radice, vai al menu di avvio su node4 e seleziona l'opzione `5` per entrare in modalità manutenzione.
+
...
Dopo aver riassegnato i dischi del nodo2 al nodo4, link:verify_node4_installation.html["verificare l'installazione di node4"].
