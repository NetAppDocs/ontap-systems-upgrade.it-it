...
[role="lead"]
Dopo aver verificato la configurazione di rete sul nodo 4, è necessario spostare i LIF dei dati NAS di proprietà del nodo 2 dal nodo 3 al nodo 4 e confermare che i LIF SAN siano presenti sul nodo 4.
...
Dopo aver portato online node4, è necessario verificare che i LIF siano integri e posizionati sulle porte corrette.
...
+
In questa fase del controllo di raggiungibilità della rete, il sistema sospende l'operazione.
...
. Se necessario, ripristinare e ripristinare i LIF spostati oppure migrare e modificare manualmente i LIF del nodo 2 che non sono riusciti a trasferirsi automaticamente nel nodo 4.
+
[role="tabbed-block"]
====
...
--
.. Migrare i LIF che non sono riusciti a trasferirsi automaticamente al nodo 4:
+
[source, cli]
----
...
Dopo aver spostato gli aggregati non root e i LIF dei dati NAS di proprietà del nodo2 dal nodo3 al nodo4, link:manage-authentication-using-kmip-servers.html["Gestire l'autenticazione tramite server KMIP"].
