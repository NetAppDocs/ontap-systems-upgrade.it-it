...
* Se stai aggiornando un sistema con dischi di archiviazione, devi completare l'intera sezione e poi passare alle link:set_fc_or_uta_uta2_config_on_node3.html#configure-fc-ports-on-node3["Configurare le porte FC sul nodo 3"] e link:set_fc_or_uta_uta2_config_on_node3.html#check-config-uta-uta2-node3["Controllare e configurare le porte UTA/UTA2 sul nodo 3"] sezioni, immettendo i comandi al prompt del cluster.
...
+
Inserisci la passphrase e le informazioni di backup che hai annotato in precedenza nella procedura. Fare riferimento a link:complete-preparation-for-upgrade.html#manage_storage_encryption_using_okm["Gestire la crittografia dello storage utilizzando Onboard Key Manager"].
...
* Se si dispone di un sistema con una configurazione FC o UTA/UTA2,link:set_fc_or_uta_uta2_config_on_node3.html["imposta e configura le porte FC o UTA/UTA2 sul nodo 3"] .
* Se non si dispone di una configurazione FC o UTA/UTA2,link:reassign-node1-disks-to-node3.html#reassign-node1-node3-app-step1["riassegnare i dischi del nodo 1 al nodo 3, passaggio 1"] in modo che node3 possa riconoscere i dischi di node1.
* Se si dispone di una configurazione MetroCluster ,link:set_fc_or_uta_uta2_config_on_node3.html["imposta e configura le porte FC o UTA/UTA2 sul nodo 3"] per rilevare i dischi collegati al nodo.
