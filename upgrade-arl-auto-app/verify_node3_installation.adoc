...
Se le porte fisiche sul nodo1 non corrispondono direttamente alle porte fisiche sul nodo3, la sezione successiva <<restore_network_configuration_on_node3,Ripristinare la configurazione di rete sul nodo 3>> deve essere utilizzata per ripristinare la connettività di rete.
...
+
Durante la procedura di ripristino della raggiungibilità, ONTAP tenta di assegnare le porte ai domini di broadcast corretti. Tuttavia, se ONTAP non riesce a determinare la raggiungibilità di una porta e questa non appartiene a nessuno dei domini di broadcast esistenti, ONTAP crea nuovi domini di broadcast per tali porte.
