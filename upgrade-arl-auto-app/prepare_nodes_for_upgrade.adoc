...
NOTE: Se si utilizza NetApp Volume Encryption (NVE) o NetApp aggregate Encryption (NAE) con Onboard Key Manager (OKM), tenere la passphrase del gestore delle chiavi pronta per completare la risincronizzazione del gestore delle chiavi in un secondo momento della procedura.
