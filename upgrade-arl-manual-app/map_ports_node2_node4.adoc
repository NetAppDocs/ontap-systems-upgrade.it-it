...
[role="lead"]
È necessario assicurarsi che le porte fisiche del nodo2 siano mappate correttamente sulle porte fisiche del nodo4. Ciò consente al nodo4 di comunicare con gli altri nodi del cluster e con la rete dopo l'aggiornamento.
...
.. Durante la riparazione della raggiungibilità, ONTAP tenta di inserire le porte nei domini di broadcast corretti. Tuttavia, se ONTAP non riesce a determinare la raggiungibilità di una porta e la porta non corrisponde a nessuno dei domini di broadcast esistenti, ONTAP tenta di creare nuovi domini di broadcast per queste porte. Se necessario, è possibile eliminare i domini di broadcast appena creati se tutte le porte che ne fanno parte diventeranno porte membri dei gruppi di interfacce. Eliminare i domini di broadcast:
+
...
. [[man_map_2_Step5]]A partire da ONTAP 9.8, ONTAP modifica automaticamente le porte home dei LIF se queste vengono spostate tra domini di broadcast durante la procedura di ripristino della raggiungibilità delle porte di rete. Se la porta home di un LIF è stata spostata su un altro nodo o non è assegnata, tale LIF viene visualizzato come LIF spostato. Ripristinare le porte home dei LIF spostati le cui porte home non esistono più o sono state ricollocate su un altro nodo.
+
...
Dopo aver mappato le porte dal nodo2 al nodo4, link:move_nas_lifs_node2_from_node3_node4_verify_san_lifs_node4.html["Spostare i LIF dei dati NAS di proprietà del node2 dal node3 al node4 e verificare i LIF SAN sul node4"].
