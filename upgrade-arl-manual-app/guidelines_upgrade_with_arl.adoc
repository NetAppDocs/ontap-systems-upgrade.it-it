...
WARNING: I passaggi necessari per preparare i controller usati con il `wipeconfig` I comandi non sono inclusi in questa procedura perché questa presuppone che l'hardware del controller sostitutivo sia nuovo e non sia stato utilizzato. È necessario contattare l'assistenza tecnica se l'hardware del controller sostitutivo è stato utilizzato in precedenza, soprattutto se i controller eseguivano Data ONTAP in modalità 7.
...
Dopo aver esaminato le linee guida per l'aggiornamento dei controller con ARL, link:required_tools_documentation.html["esaminare gli strumenti e la documentazione necessari"].
