...
+
Se né il nodo1 né il nodo2 possiedono aggregati di cui sono il proprietario corrente (ma non il proprietario principale), il sistema restituisce un messaggio simile al seguente esempio:
...
+
Per informazioni dettagliate sul  `network interface revert` comando, fare riferimento a link:other_references.html["Riferimenti"] per collegarsi al _ONTAP 9 Command reference_.
...
+
Le VLAN vengono configurate sulle porte fisiche. Se le porte fisiche cambiano, le VLAN dovranno essere ricreate in un secondo momento.
...
+
Se né iSCSI né FC sono configurati per l'SVM, il comando visualizza un messaggio simile al seguente esempio:
...
+
Se iSCSI o FC sono configurati per l'SVM, il comando visualizza un messaggio simile al seguente esempio:
...
+
Potresti voler riutilizzare i parametri di rete dei dispositivi di gestione remota, in questo caso gli SP, dal sistema originale per gli SP sui nuovi nodi. Per informazioni dettagliate sugli SP, fare riferimento a link:other_references.html["Riferimenti"] _System Administration Reference_ e _ONTAP 9 Command reference_.
...
+
Per informazioni dettagliate su AutoSupport, fare riferimento a link:other_references.html["Riferimenti"] per collegarsi al _System Administration Reference_ e al _ONTAP 9 Command reference_.
...
Dopo aver preparato i nodi per l'aggiornamento, link:manage_authentication_okm.html["Gestisci le chiavi di autenticazione tramite Onboard Key Manager"].
