...
Durante la procedura di aggiornamento, le LIF remote gestiscono il traffico verso le LUN SAN. Lo spostamento delle LIF SAN non è necessario per la salute del cluster o dei servizi durante l'aggiornamento. Le LIF SAN non vengono spostate a meno che non sia necessario mapparle su nuove porte. È necessario verificare che le LIF siano integre e posizionate sulle porte appropriate dopo aver portato online node3.
...
Dopo aver spostato i LIF dei dati NAS di proprietà del nodo1 dal nodo2 al nodo3 e aver verificato i LIF SAN sul nodo3, .
