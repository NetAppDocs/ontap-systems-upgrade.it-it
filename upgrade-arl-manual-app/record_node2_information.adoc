...
NOTE: Il tuo sistema potrebbe non disporre di LIF intercluster. Hai una LIF di gestione del cluster solo su un nodo di una coppia di nodi. Una LIF di gestione del cluster è visualizzata nell'output di esempio di link:record_node1_information.html#step1["Fase 1"] in _Record node1 port information_.
...
+
In una fase successiva della procedura, i domini di broadcast dovranno essere mappati alle porte del nuovo controller.
...
Dopo aver registrato le informazioni del nodo2, link:retire_node2.html["ritirare node2"].
