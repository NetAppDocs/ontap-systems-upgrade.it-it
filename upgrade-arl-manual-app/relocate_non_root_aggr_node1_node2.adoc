...
.Fasi
. [[relocate-step1]]Riposizionare gli aggregati non radice completando i seguenti sottopassaggi:
+
...
+
L'output visualizza  `Done` per un aggregato dopo che è stato riposizionato.
...
Fare riferimento a link:other_references.html["Riferimenti"] per collegarsi al contenuto _Gestione dischi e aggregati con la CLI_ e al _Riferimento ai comandi ONTAP 9_ per ulteriori informazioni sui comandi di rilocazione degli aggregati di archiviazione.
...
Dopo aver spostato gli aggregati non root dal nodo1 al nodo2, link:move_nas_lifs_node1_node2.html["spostare i LIF dei dati NAS di proprietà del node1 sul node2"].
