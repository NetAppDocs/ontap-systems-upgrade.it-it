...
summary: Risolvere i problemi di riposizionamento degli aggregati che potrebbero verificarsi durante un aggiornamento ARL manuale sui controller che eseguono ONTAP 9.8 o versioni successive. 
---
...
+
Per informazioni dettagliate sulle opzioni  `storage aggregate relocation start`,  `override-vetoes` e  `override-destination-checks`, fare riferimento a link:other_references.html["Riferimenti"] per il collegamento al _ONTAP 9 Command reference_.
...
* Durante la Fase 3, gli aggregati vengono spostati dal nodo2 al nodo3. Alcuni degli aggregati spostati hanno node1 come nodo home. Ad esempio, un aggregato di questo tipo potrebbe essere chiamato aggr_node_1. Se la rilocazione di aggr_node_1 fallisce durante la Fase 3 e la rilocazione non può essere forzata, allora l'aggregato rimane su node2.
* Dopo la fase 4, quando il nodo2 viene sostituito con il nodo4. Quando il nodo2 viene sostituito, aggr_node_1 va online con il nodo4 come porta home invece del nodo3.
