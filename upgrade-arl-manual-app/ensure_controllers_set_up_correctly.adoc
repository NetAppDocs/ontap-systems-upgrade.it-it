...
+
Vai a link:other_references.html["Riferimenti"] per collegarti al _System Administration Reference_ per informazioni sugli SP e all'_ONTAP 9 Command reference_ per informazioni dettagliate sul comando  `system service- processor network modify`.
...
Dopo aver verificato che i nuovi controller siano configurati correttamente, link:set_up_storage_encryption_new_controller.html["Configurare la crittografia dell'archiviazione sul nuovo modulo controller"].
