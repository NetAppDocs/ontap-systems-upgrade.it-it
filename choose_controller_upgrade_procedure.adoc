...
---
= Scegli una procedura di aggiornamento per l'hardware del controller AFF, ASA o FAS
:allow-uri-read: 
:icons: font
:imagesdir: ./media/
...
[role="lead"]
In generale, il modo in cui aggiorni l'hardware del controller NetApp AFF, ASA o controller del FAS dipende dai modelli di piattaforma dei nodi originali. Puoi aggiornare spostando gli aggregati o i volumi (entrambe procedure non interruttive), oppure spostando lo storage (una procedura interruttiva). Se hai versioni diverse di ONTAP in esecuzione sui nodi originali e su quelli nuovi, potresti dover eseguire un aggiornamento software prima di iniziare l'aggiornamento hardware.
...
| Sto utilizzando una versione software diversa sui nodi originali e su quelli nuovi? | Aggiornare la versione di ONTAP sui nodi originali alla versione più recente supportata. Quando possibile, i nuovi nodi devono supportare la versione di ONTAP in esecuzione sui nodi originali. Se ciò non è possibile, link:https://docs.netapp.com/us-en/ontap/upgrade/prepare.html["Aggiornare la versione ONTAP"^] sui nodi originali alla versione più recente supportata. Non è possibile avere una differenza di versione superiore a quattro tra i nodi originali e quelli nuovi. Ad esempio, ONTAP 9.8 e 9.12.1 sono supportati; tuttavia, ONTAP 9.8 e 9.13.1 non sono supportati. link:https://docs.netapp.com/us-en/ontap/upgrade/concept_mixed_version_requirements.html["Scopri di più sui cluster ONTAP con versioni miste"^]. 


| Voglio effettuare un aggiornamento dirompente? | link:upgrade/upgrade-decide-to-use-this-guide.html["Eseguire l'upgrade spostando lo storage"]. Ciò richiede un nuovo sistema con un proprio storage e ripiani per archiviare i dati del sistema originale. È possibile convertire un ripiano del controller per determinati aggiornamenti. 
...
* Convertire il sistema originale in uno shelf di dischi e spostare lo storage:
+
** link:upgrade/upgrade_aff_a250_to_aff_a400_ndu_upgrade_workflow.html["Convertire AFF A250 in uno shelf di dischi e spostare lo storage su AFF A400"]
** link:upgrade/convert-to-drive-shelf.html["Convertire AFF A150, AFF A220 o FAS2820 in uno shelf di dischi e spostare lo storage su un sistema sostitutivo supportato"]
...
| Ho un sistema modulare? *Nota*: in un sistema modulare, il controller non ha dischi interni; tutto lo storage è in shelf collegati esternamente. | link:upgrade-arl/index.html["Aggiornamento mediante trasferimento degli aggregati"]. 
...
| Ho un sistema integrato e voglio aggiornarlo sostituendo solo il modulo controller, mantenendo lo chassis e i dischi esistenti? | link:upgrade-arl-auto-in-chassis/index.html["Aggiorna utilizzando ARL, mantenendo lo stesso chassis, gli stessi dischi e i dati memorizzati sui dischi"]. 
...
Questo esempio fornisce possibili risposte per un aggiornamento del controller FAS9500 a FAS90.
